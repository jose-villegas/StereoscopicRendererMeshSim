...
void main()
{
    ...
    for(int i = 0; i < light.count; i++) {
        ...
        if(light.source[i].lightType == LIGHT_SPOT) {
            accumColor += calculateSpotLight(light.source[i], position, normalFromMap, diffuseColor.rgb, specularColor.rgb);
        } else if(light.source[i].lightType == LIGHT_DIRECTIONAL) {
            accumColor += calculateDirectionalLight(light.source[i], position, normalFromMap, diffuseColor.rgb, specularColor.rgb);
        } else {
            accumColor += calculatePointLight(light.source[i], position, normalFromMap, diffuseColor.rgb, specularColor.rgb);
        }
    }
    // correct gama values
